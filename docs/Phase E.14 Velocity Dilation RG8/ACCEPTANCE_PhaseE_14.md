...
> **状态**：✅ **实现完成 + 静态/Lua 语法验证通过，待 CI 6 平台 + Windows runtime smoke 验证**
...
| T7 Static verification + CI | `git diff --check` 已通过；`lightc -p` `hdr.lua` + `demo_ssr/main.lua` exit 0；CI 6 平台 + Windows runtime smoke 待用户提交后触发 | 🟡 |
...
| GitHub Actions 6 平台 build | ⏳ | 待提交后触发 |
| Windows runtime smoke `hdr.lua` | ⏳ | 待 CI |
...
- GitHub Actions 6 平台 build success ⏳ 待提交
- Windows runtime smoke `hdr.lua` 16 个表面 + 6 段 §8 0 fail ⏳ 待 CI
- 真实窗口环境下，开 dilation 后 Temporal SSR 几何边缘 halo 减弱；切到 RG8 后 VRAM 减少且视觉差异可接受 ⏳ 用户验证
