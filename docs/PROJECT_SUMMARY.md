...
| **窗口** | SDL3 | 跨平台窗口/事件/GL 上下文 (替代 GLFW) |
...
| **物理** | Box2D v2.4.1 | 刚体模拟 + 碰撞检测 (桌面/移动平台) |
| **网络** | libuv + POSIX socket | HTTP 1.1 + WebSocket (RFC 6455) |
...
| `Light.Input` | `luaopen_Light_Input` | 统一输入 (键盘/鼠标/触摸/手柄) | C++ |
| `Light.Physics` | `luaopen_Light_Physics` | Box2D v2.4.1 物理引擎 (桌面/移动) | C++ |
| `Light.ECS` | `luaopen_Light_ECS` | 实体组件系统 | C++ |
| `Light.Graphics.Particles` | `luaopen_Light_Graphics_Particles` | 2D 粒子系统 | C++ |
| `Light.Graphics.Tilemap` | `luaopen_Light_Graphics_Tilemap` | 瓦片地图 | C++ |
...
### 4.3 窗口管理 (`light_ui.cpp` + `platform_window_sdl3.cpp`)

**Window 模块** (13 函数, 基于 SDL3):

```
Open(w, h, title)      — 创建 SDL3 窗口 (支持 1-3 参数重载)
Close()                — 关闭窗口
ID()                   — 获取原生窗口句柄
Get/SetWidth           — 宽度存取
Get/SetHeight          — 高度存取
...
SetOrientation(mode)   — 屏幕方向 (Android JNI)
__call()               — 事件循环步进
```

**事件回调**: SDL3 拉模式事件 → PlatformWindow::Event → 分发到 Input 模块 + Lua 回调。
...
- ✅ **渲染管线**: GL 3.3 Core + GLES 3.0 Shader 管线 + Legacy 回退
- ✅ **窗口层**: SDL3 替代 GLFW，原生 Android/iOS 支持
- ✅ **物理引擎**: Box2D v2.4.1 (桌面/移动平台)
- ✅ **ECS**: 轻量 Entity-Component-System
- ✅ **粒子系统**: Light.Graphics.Particles
- ✅ **瓦片地图**: Light.Graphics.Tilemap
- ✅ **输入管理器**: 键盘/鼠标/触摸/手柄 + 动作映射
- ✅ **跨平台音频**: miniaudio 替代 PlaySound
- ✅ **跨平台网络**: libuv + POSIX socket 移动端回退

## 12. 待改进方向 (Phase 3+)

- **用户 Shader API**: 暴露 Shader 编译/绑定/Uniform 接口
- **精灵动画**: SpriteAnimation 帧动画系统
- **UI 控件库**: Button/Label/TextInput 等基础控件
- **场景管理器**: Scene Stack 场景切换
- **资源热重载**: 文件监视 + 自动重载
- **Box2D Android 适配**: vendored Box2D v2.4.1
- **Vulkan / Metal 渲染后端**
- **Lumen JIT 编译器**
- **AES-256 加密替代 XOR**
