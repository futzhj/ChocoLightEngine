#ifdef __ANDROID__
// ==================== Android 精简版 ====================
// 完整版 (~470行) 的代码/数据量会改变 .so 段布局,
// 触发 MuMu 模拟器 RenderThread 的 destroyed-mutex 崩溃.
// 此精简版提供键盘/鼠标/触摸的完整功能, 手柄/动作映射返回空值.
#include "light.h"
#include "platform_window.h"
#include <cstring>

static constexpr int AMAX_KEYS = 512;
static constexpr int AMAX_MOUSE = 8;
static constexpr int AMAX_TOUCHES = 10;

struct ATouchState { int id; float x, y, pressure; bool active; };

static bool  a_keys[AMAX_KEYS] = {};
static bool  a_keysPrev[AMAX_KEYS] = {};
static bool  a_mouse[AMAX_MOUSE] = {};
static float a_mx = 0, a_my = 0, a_wdx = 0, a_wdy = 0;
static ATouchState a_touches[AMAX_TOUCHES] = {};
static int a_touchCount = 0;

void InputProcessEvent(const PlatformWindow::Event& ev) {
    switch (ev.type) {
        case PlatformWindow::Event::KeyDown:
            if (ev.keycode >= 0 && ev.keycode < AMAX_KEYS) a_keys[ev.keycode] = true; break;
        case PlatformWindow::Event::KeyUp:
            if (ev.keycode >= 0 && ev.keycode < AMAX_KEYS) a_keys[ev.keycode] = false; break;
        case PlatformWindow::Event::MouseDown:
            if (ev.button >= 0 && ev.button < AMAX_MOUSE) a_mouse[ev.button] = true; break;
        ...
        case PlatformWindow::Event::TouchUp:
            for (int i = 0; i < a_touchCount; i++) {
                if (a_touches[i].id == ev.touchId) {
                    a_touches[i] = a_touches[--a_touchCount]; break;
                }
            } break;
        default: break;
    }
...
static int al_IsKeyDown(lua_State* L)     { int k = (int)luaL_checkinteger(L,1); lua_pushboolean(L, k>=0&&k<AMAX_KEYS&&a_keys[k]); return 1; }
static int al_IsKeyPressed(lua_State* L)  { int k = (int)luaL_checkinteger(L,1); lua_pushboolean(L, k>=0&&k<AMAX_KEYS&&a_keys[k]&&!a_keysPrev[k]); return 1; }
static int al_IsMouseDown(lua_State* L)   { int b = (int)luaL_checkinteger(L,1); lua_pushboolean(L, b>=0&&b<AMAX_MOUSE&&a_mouse[b]); return 1; }
static int al_GetMousePos(lua_State* L)   { lua_pushnumber(L,a_mx); lua_pushnumber(L,a_my); return 2; }
static int al_GetMouseWheel(lua_State* L) { lua_pushnumber(L,a_wdx); lua_pushnumber(L,a_wdy); return 2; }
static int al_GetTouchCount(lua_State* L) { lua_pushinteger(L,a_touchCount); return 1; }
static int al_GetTouch(lua_State* L) {
    int i = (int)luaL_checkinteger(L,1) - 1;
    if (i<0||i>=a_touchCount) { lua_pushinteger(L,0); lua_pushnumber(L,0); lua_pushnumber(L,0); lua_pushnumber(L,0); return 4; }
    lua_pushinteger(L,a_touches[i].id); lua_pushnumber(L,a_touches[i].x); lua_pushnumber(L,a_touches[i].y); lua_pushnumber(L,a_touches[i].pressure); return 4;
}
static int al_Update(lua_State*) { memcpy(a_keysPrev,a_keys,sizeof(a_keys)); a_wdx=a_wdy=0; return 0; }
static int al_stub0(lua_State* L) { lua_pushinteger(L,0); return 1; }
static int al_stubF(lua_State* L) { lua_pushboolean(L,0); return 1; }
static int al_stubS(lua_State* L) { lua_pushstring(L,""); return 1; }
static int al_stubN(lua_State*  ) { return 0; }

static const luaL_Reg a_input_funcs[] = {
    {"IsKeyDown",al_IsKeyDown},{"IsKeyPressed",al_IsKeyPressed},
    {"IsMouseDown",al_IsMouseDown},{"GetMousePosition",al_GetMousePos},
    {"GetMouseWheel",al_GetMouseWheel},{"GetTouchCount",al_GetTouchCount},
    {"GetTouch",al_GetTouch},{"Update",al_Update},
    {"GetGamepadCount",al_stub0},{"IsGamepadConnected",al_stubF},
    {"GetGamepadButton",al_stubF},{"GetGamepadAxis",al_stub0},
    {"GetGamepadName",al_stubS},{"AddAction",al_stubN},{"IsActionDown",al_stubF},
    {NULL,NULL}
};

int luaopen_Light_Input(lua_State* L) {
    memset(a_keys,0,sizeof(a_keys)); memset(a_keysPrev,0,sizeof(a_keysPrev));
    memset(a_mouse,0,sizeof(a_mouse)); memset(a_touches,0,sizeof(a_touches));
    a_touchCount=0; a_mx=a_my=a_wdx=a_wdy=0;
    LT::RegisterModule(L, "Input", a_input_funcs);
    return 1;
}
#else
/**
