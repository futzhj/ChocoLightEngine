...
// ==================== Phase F.2.2 — Multi-Instance API ====================
...
int CreateInstance() {
    for (int i = 1; i < MAX_INSTANCES; ++i) {
        if (!g_slot_in_use[i]) {
            ...
            ++g_count;
            CC::Log(CC::LOG_INFO,
                    "LensFlareRenderer::CreateInstance: id=%d (count=%d)", i, g_count);
            return i;
        }
    }
    CC::Log(CC::LOG_WARN,
            "LensFlareRenderer::CreateInstance: 槽位已满 (MAX=%d)", MAX_INSTANCES);
    return 0;
}
...
bool DestroyInstance(int id) {
    ...
    if (!g_slot_in_use[id]) {
        CC::Log(CC::LOG_WARN, "LensFlareRenderer::DestroyInstance: id=%d 未分配", id);
        return false;
    }
    ...
    CC::Log(CC::LOG_INFO,
            "LensFlareRenderer::DestroyInstance: id=%d (count=%d, active=%d)",
            id, g_count, g_active);
    return true;
}
...
int CloneInstance(int srcId) {
    ...
    for (int i = 1; i < MAX_INSTANCES; ++i) {
        if (!g_slot_in_use[i]) {
            g_states[i] = g_states[srcId];      // 复制全部参数 + flareTexId
            ...
            ++g_count;
            CC::Log(CC::LOG_INFO,
                    "LensFlareRenderer::CloneInstance: srcId=%d -> id=%d (count=%d)",
                    srcId, i, g_count);
            return i;
        }
    }
    CC::Log(CC::LOG_WARN, "LensFlareRenderer::CloneInstance: 槽位已满");
    return 0;
}
