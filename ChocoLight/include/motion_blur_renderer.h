...
// 每 instance 各自独立 motionBlur RT + 参数 (strength/sampleCount/mode/halfRes).
// 切 active instance 后, 现有 namespace fn 自动作用于该 instance.
// 用途: split-screen 4 player 各自 motion blur profile (强度/采样/halfRes 全独立).
//
// 与 HDR/TAA/Bloom multi-instance API 命名/语义完全一致.

/// 创建新 MotionBlur instance, 返 id [1, 3] / 0 (槽满)
int CreateInstance();
