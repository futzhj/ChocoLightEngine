/**
 * tiny_aes.c — 紧凑 AES 实现
 * 公共领域 (Public Domain), 基于 kokke/tiny-AES-c 的标准 AES-NIST 实现
 *
 * 实现细节:
 *   - SubBytes / ShiftRows / MixColumns / AddRoundKey 标准操作
 *   - Key Expansion 支持 AES-128/192/256
 *   - ECB 模式: 单块独立加密
 *   - CBC 模式: 块间链式异或 (IV 链式更新)
 */
...
static void InvCipher(state_t* state, const uint8_t* RoundKey) {
    AddRoundKey(AES_NR, state, RoundKey);
    for (uint8_t round = (AES_NR - 1); round > 0; --round) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(round, state, RoundKey);
        InvMixColumns(state);
...
static void XorWithIv(uint8_t* buf, const uint8_t* Iv) {
    for (uint8_t i = 0; i < AES_BLOCKLEN; ++i) buf[i] ^= Iv[i];
}
